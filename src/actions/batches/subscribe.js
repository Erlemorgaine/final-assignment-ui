export const BATCH_CREATED = 'BATCH_CREATED'
export const BATCH_UPDATED = 'BATCH_UPDATED'
export const BATCH_REMOVED = 'BATCH_REMOVED'
export const BATCH_STUDENTS_UPDATED = 'BATCH_STUDENTS_UPDATED'
export const BATCH_STUDENT_UPDATED = 'BATCH_STUDENT_UPDATED'
export const BATCH_STUDENT_EVALUATIONS_UPDATED = 'BATCH_STUDENT_EVALUATIONS_UPDATED'
export const BATCH_STUDENT_EVALUATION_UPDATED = 'BATCH_STUDENT_EVALUATION_UPDATED'
